...
function addToCart(productId) {
  ...
  if (existingProduct) {
    ...
    showToast(
      `Đã có ${existingProduct.quantity} ${product.name} trong giỏ hàng`
    );
  } else {
...
function kiemtradangnhap() {
  ...
  if (currentUser != null) {
    ...
    ).innerHTML = `<li><a onclick="myAccount()"><i class="fa-light fa-circle-user"></i> Tài khoản của tôi</a></li>
  ...
  productList.forEach((product) => {
    const productHTML = `
        <div class="item" data-id="${product.id}">
            <img src="${getPathImage(product.image)}" alt="${
      product.name
...
function openProductDetail(productId) {
  ...
  document
    .getElementById("product-detail-image")
    .setAttribute("src", `${getPathImage(product.image)}`);
  document.getElementById("product-detail-name").textContent = product.name;
  document.getElementById("product-detail-price").textContent = formatCurrency(
    product.price
  );
  ...
  paginatedProducts.forEach((product) => {
    const productHTML = `
      ...
      product.name
                ...
                <h2 onclick="openProductDetail(${product.id})">${
      product.name
    }</h2>
                <div class="price">${formatCurrency(product.price)}</div>
...
document.querySelector(".checkout-btn").addEventListener("click", (e) => {
  if (
    document
      .querySelector(".auth-container .text-tk")
...
document.querySelector(".buyNow").addEventListener("click", (e) => {
  if (
    ...
    let cart_buy_now = localStorage.getItem("cart_buy_now")
      ? JSON.parse(localStorage.getItem("cart_buy_now"))
      : [];
...
advancedSearchFormSubmitButton.onclick = function (e) {
  ...
  if (productName === "") {
    let error = getErrorElement("Tên trống");
    form["product-name"].insertAdjacentElement("afterend", error);
    shouldStop = true;
  }

  if (productLowestPrice === ""||productHighestPrice === "") {
    let error = getErrorElement("Giá trống");
    form["product-lowest-price"&&"product-highest-price"].insertAdjacentElement("afterend", error);
    shouldStop = true;
  } else if (!/^\d+$/.test(productLowestPrice)) {
    let error = getErrorElement("Phải là số thực dương");
  ...
  if (parseFloat(productHighestPrice) < parseFloat(productLowestPrice)) {
    let error = getErrorElement("Giá cao thấp hơn giá thấp");
    form["product-highest-price"].insertAdjacentElement("afterend", error);
    shouldStop = true;
  }
  if (shouldStop) {
